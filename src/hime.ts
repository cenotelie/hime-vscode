 ...
 */
export function compileGrammar(context: VSCode.ExtensionContext, fileUri: string, grammar: string, observer: ProcessObserver, parameters: string[]) {
    ...
    if (process.platform === "win32") {
        child = ChildProcess.spawn(
            Path.resolve(context.extensionPath, "target", "bin", "net461", "himecc.exe"),
 ...
 */
export function parseInput(context: VSCode.ExtensionContext, assemblyFile: string, parserQName: string, input: string): Promise<ParseResult> {
    return new Promise((resolve, reject) => {
        let options = { cwd: VSCode.workspace.rootPath };
        var child: ChildProcess.ChildProcess = null;
        if (process.platform === "win32") {
            child = ChildProcess.spawn(
                Path.resolve(context.extensionPath, "target", "bin", "net461", "parseit.exe"),
