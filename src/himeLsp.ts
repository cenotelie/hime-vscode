...
export function createLanguageClient(
  ...
  function createServer(): Promise<StreamInfo> {
    let serverType = VSCode.workspace
      .getConfiguration("hime")
      .get("lsp.server");
    if (serverType == "remote") {
      let port = VSCode.workspace
        .getConfiguration("hime")
        .get("lsp.server.port") as number;
...
function serverLaunchProcess(
  ...
  return new Promise((resolve, reject) => {
    client.outputChannel.appendLine(
      "[INFO] Creating a new hime language server with a new process ..."
    );
    ...
    resolveJavaGetVersion(java)
      .then(
        (version) => {
          client.outputChannel.appendLine(
            "[INFO] Will use Java: " + java + " (version " + version + ")"
          );
          let jarPath = Path.resolve(
          ...
          client.outputChannel.appendLine(
            "[INFO] Launching server as " + java + " -jar " + jarPath
          );
...
function serverConnect(
  ...
  return new Promise((resolve, reject) => {
    client.outputChannel.appendLine(
      "[INFO] Connecting to hime language server on port " + port + " ..."
    );
...
function resolveJava(): string {
  ...
  let settingJavaHome = VSCode.workspace
    .getConfiguration("hime")
    .get("java") as string;
...
function resolveJavaGetVersion(execName: string): Promise<string> {
  return new Promise((resolve, reject) => {
    let result = ChildProcess.execFile(
      execName,
      ["-version"],
      {},
      (error, stdout, stderr) => {
